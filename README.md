...
🔍 Intent-Aware Search: * Direct Mode: Instant lookup for specific verses (e.g., "2.47").
...
LLM: Gemini 2.5 Flash — Chosen for its deep reasoning, multilingual support, and massive 1M+ token context window.

Embeddings: all-MiniLM-L6-v2 (via HuggingFace) — A local, CPU-efficient model that turns text into 384-dimensional vectors for semantic search.

The Backbone (Software & Infrastructure)
LangChain: Orchestrates the complex RAG pipeline.

ChromaDB: A specialized vector database that stores shloka "embeddings" for lightning-fast similarity search.

HuggingFace Datasets: Powers the raw data pipeline (JDhruv14/Bhagavad-Gita_Dataset).
...
Bash
git clone https://github.com/your-username/gyan-bharatam.git
cd gyan-bharatam
Environment Setup:
...
Bash
python app.py
🏆 Project Impact (Internship Context)
Built during my AI/ML internship to solve the problem of AI Hallucinations in Cultural Datasets. By utilizing RAG, this system achieves a 99% accuracy rate in direct verse citation compared to the 60% baseline of standard LLMs.